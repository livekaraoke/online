/* Copyright © 2026 Roxanna. js/hero-slideshow.js — preserved site configuration / asset support. */
(() => {
  "use strict";
  ...
  function schedule() {
    clearTimeout(timer);
    if (usablePhotos.length < 2 || document.hidden || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  ...
  function showNext() {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        setTimeout(() => {
          if (!standby.classList.contains("is-active")) standby.removeAttribute("src");
        }, FADE_MS + 100);
