...
(() => {
  ...
  // Gallery images are local files only. Ignore remote/parent paths rather than
  // falling back to hero images or loading a different act's media.
  const galleryPhotos=(Array.isArray(cfg.photos)?cfg.photos:[]).filter(photo =>
    photo && /^assets\/gallery\/[a-zA-Z0-9_-][a-zA-Z0-9_. -]*\.(?:png|jpe?g|webp|avif|gif)$/i.test(photo.src)
  );
  let currentPhotoIndex=0;
  function renderAllPhotos(){
    const thumbnail=(photo,index,cls)=>`<button class="${cls}" type="button" data-photo-index="${index}" aria-label="Open photo ${index+1}"><img src="${escapeHTML(photo.src)}" alt="${escapeHTML(photo.alt||'Roxanna live')}" loading="lazy" decoding="async"></button>`;
    const grid=$("allPhotosGrid");
    if(grid)grid.innerHTML=galleryPhotos.map((photo,index)=>thumbnail(photo,index,'all-photo-thumb')).join("")||'<p class="muted">Photos coming soon.</p>';
    const preview=$("photoPreviewGrid");
    if(preview)preview.innerHTML=galleryPhotos.slice(0,3).map((photo,index)=>thumbnail(photo,index,'photo-thumb')).join("")||'<p class="muted">Photos coming soon.</p>';
    $("viewMorePhotosBtn").hidden=galleryPhotos.length===0;
  }
