...
function setup(){
  ...
  window.LS26Settings={get:()=>({metronomeBeat1Color:'#ffd05a',metronomeBeat2Color:'#00cafa',metronomeBeat3Color:'#00cafa',metronomeBeat4Color:'#00cafa',metronomeFlashBrightness:72,metronomeEdgeThickness:8,metronomeFlashDuration:120,metronomeShowBeatNumber:true,metronomeNumberSize:220,metronomeNumberOpacity:42,metronomeNumberVerticalPosition:31})};
...
(async()=>{
  ...
  env.audio().currentTime=.06;env.runFrame();
  const visualLayer=el('lvMetroScreenBeat');
  assert.equal(visualLayer.classList.contains('pulse'),true,'scroll-started click flashes the screen edges');
  assert.equal(visualLayer.classList.contains('beat-one'),true,'beat 1 enables top and bottom edge flashes');
  assert.equal(visualLayer.classList.contains('accented'),true,'accented first beat keeps distinct stronger styling');
  assert.equal(visualLayer.querySelectorAll('.lv-metro-screen-number').length,1,'only one large beat number is rendered');
  assert.equal(visualLayer.querySelector('.lv-metro-screen-number').textContent,'1');
  assert.equal(visualLayer.querySelectorAll('.lv-metro-screen-edge').length,4,'four edge elements remain available, with CSS limiting beats 2-4 to left/right');
  ...
  console.log('PASS: placement, startup preference, independent/linked playback, side-only visual beats with beat-1 top/bottom and one large number, tap BPM sync, end/hidden-page stop and no autoplay.');
