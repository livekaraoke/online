/* Copyright © 2026 LiveSuite. All rights reserved.
 * oldadmin/host/lyrics/lyrics-data/houseoftherisingsun.js — preserved application behaviour and compatibility support.
 * Original notices and functionality retained below. See FUNCTIONS.txt.
 */
window.songData = {
  "id": "houseoftherisingsun",
  "title": "The House of the rising sun",
  "artist": "The Animals",
  "lyrics": [
    {
      "type": "note",
      "text": "INTRO Instrumental (wait for signal)"
    },
    {
      "type": "section",
      "text": "VERSE 1"
    },
    {
      "type": "verse",
      "lines": [
        "There is a house in New Orleans,",
        "They call the Rising Sun.",
        "",
        "And it's been the ruin of many a poor boy,",
        "And God I know, I'm one."
      ]
    },
    {
      "type": "note",
      "text": "Short Instrumental (wait for signal)"
    },
    {
      "type": "section",
      "text": "VERSE 2"
    },
    {
      "type": "chorus",
      "lines": [
        "My mother was a tailor,",
        "She sewed my new blue jeans.",
        "",
        "My father was a gamblin' man,",
        "Down in New Orleans."
      ]
    },
    {
      "type": "note",
      "text": "Short Instrumental (wait for signal)"
    },
    {
      "type": "section",
      "text": "VERSE 3"
    },
    {
      "type": "verse",
      "lines": [
        "Now the only thing a gambler needs,",
        "Is a suitcase and a trunk.",
        "",
        "And the only time he's satisfied,",
        "Is when he's all drunk."
      ]
    },
    {
      "type": "section-seperator"
    },
    {
      "type": "action",
      "text": "(Long Instrumental)"
    },
    {
      "type": "note",
      "text": "(wait for signal)"
    },
    {
      "type": "section-seperator"
    },
    {
      "type": "section",
      "text": "VERSE 4"
    },
    {
      "type": "verse",
      "lines": [
        "Oh mother, tell your children,",
        "Not to do what I have done.",
        "",
        "Spend your lives in sin and misery,",
        "In the House of the Rising Sun."
      ]
    },
    {
      "type": "note",
      "text": "Short Instrumental (wait for signal)"
    },
    {
      "type": "section",
      "text": "VERSE 5"
    },
    {
      "type": "chorus",
      "lines": [
        "Well, I got one foot on the platform,",
        "The other foot on the train.",
        "",
        "I'm goin' back to New Orleans,",
        "To wear that ball and chain."
      ]
    },
    {
      "type": "note",
      "text": "Short Instrumental (wait for signal)"
    },
    {
      "type": "section",
      "text": "VERSE 6"
    },
    {
      "type": "verse",
      "lines": [
        "Well, there is a house in New Orleans,",
        "They call the Rising Sun.",
        "",
        "And it's been the ruin of many a poor boy,",
        "And God I know, I'm one."
      ]
    },
    {
      "type": "note",
      "text": "Ending (short instrumental)"
    },
    {
      "type": "action",
      "text": "[ END ]"
    }
  ]
};