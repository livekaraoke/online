 ...
 * host/js/lyricscreator.js — preserved application behaviour and compatibility support.
...
(() => {
  ...
  const BUILTIN_SECTION_TITLE_DEFAULTS = {
    verse: "#ffffff",
    preChorus: "#ffb45c",
    chorus: "#42f35c",
    postChorus: "#ffffff",
  ...
  let sectionTitleDefaults = { ...BUILTIN_SECTION_TITLE_DEFAULTS };
  ...
  function getEffectiveSectionTitleColour(section) {
    return section?.style?.titleColor || getSystemSectionTitleColour(section?.title);
  }

  async function loadSectionTitleDefaults() {
    try {
      const snap = await db.collection("noteSettings").doc("lyricsCreatorSectionTitleDefaults").get();
      if (snap.exists) sectionTitleDefaults = { ...BUILTIN_SECTION_TITLE_DEFAULTS, ...(snap.data() || {}) };
    } catch (error) {
  ...
  async function saveSectionTitleDefaults() {
    ...
    await db.collection("noteSettings").doc("lyricsCreatorSectionTitleDefaults").set({
      ...sectionTitleDefaults,
  ...
  async function resetSectionTitleDefaults() {
    const ok = await showConfirm("Reset Section Defaults?", "Reset the system section-title colours to their built-in defaults?");
    if (!ok) return;
    sectionTitleDefaults = { ...BUILTIN_SECTION_TITLE_DEFAULTS };
    await db.collection("noteSettings").doc("lyricsCreatorSectionTitleDefaults").set({
      ...sectionTitleDefaults,
      updatedAt: firebase.firestore.FieldValue.serverTimestamp()
    }, { merge: true });
  ...
  function performUndo() {
    ...
    restoreEditorState(undoStack[undoStack.length - 1]);
  }
  ...
  function markDirty() {
    ...
    if (!historyApplying) {
      ...
      markDirty._historyTimer = setTimeout(pushUndoState, 120);
    }
  }
  ...
  async function loadLinkSongs() {
    if (linkSongs.length) return linkSongs;
  ...
  async function openLinkModal(editor) {
    captureSelection(editor);

    const select = $("linkSongSelect");
    const label = $("linkLabelInput");
    ...
    try {
      const songs = await loadLinkSongs();
      ...
      console.error("Could not load songs for link:", error);
      select.innerHTML = `<option value="">Could not load songs</option>`;
    }
  }

  function insertSelectedSongLink() {
    const songId = $("linkSongSelect").value;
    if (!songId) return;

    const song = linkSongs.find(item => item.id === songId);
    if (!song) return;

    const customLabel = $("linkLabelInput").value.trim();
    const linkLabel = customLabel || `${song.title}${song.artist ? ` - ${song.artist}` : ""}`;

    const html =
      `<a class="lyrics-song-link" href="lyricview.html?id=${encodeURIComponent(song.id)}" ` +
      `data-song-link="${esc(song.id)}">${esc(linkLabel)}</a>`;
  ...
  document.addEventListener("click", async event => {
    ...
    const link = event.target.closest("[data-insert-link]");
    if (link) {
      const editor = link.closest(".creator-section-body")?.querySelector(".creator-rich-editor");
  ...
  $("insertLinkCancelBtn").onclick = () => $("linkModal").classList.add("hidden");
  $("insertLinkConfirmBtn").onclick = insertSelectedSongLink;
  $("linkSongSelect").addEventListener("change", () => {
    const song = linkSongs.find(item => item.id === $("linkSongSelect").value);
    if (song && !$("linkLabelInput").value.trim()) {
