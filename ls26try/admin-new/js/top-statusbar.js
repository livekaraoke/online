 ...
 * admin-new/js/top-statusbar.js — preserved application behaviour and compatibility support.
...
(function () {
  ...
  function renderSessionNotes() {
    ...
    if (document.activeElement !== notesInput) {
      notesInput.value = localStorage.getItem("ls26:notes:"+firebase.app().options.projectId+":"+currentSession.id) ?? currentSession.notes ?? "";
    }
  
    if (status && !status.dataset.saving) {
      status.innerText = localStorage.getItem("ls26:notes:"+firebase.app().options.projectId+":"+currentSession.id)!==null ? "Unsynced draft restored — type to save" : "Changes save automatically";
    }
  }
  ...
  function saveSessionNotesDelayed() {
    ...
    try{localStorage.setItem('ls26:notes:'+firebase.app().options.projectId+':'+currentSession.id,notesInput.value);}catch(_){}
    const capturedSessionId=currentSession.id,capturedNotes=notesInput.value;
    notesSaveTimer = setTimeout(()=>saveSessionNotesNow(capturedSessionId,capturedNotes), 5000);
  }
  
  async function saveSessionNotesNow(capturedSessionId,capturedNotes) {
    const dbRef = getDb();
    const notesInput = $("tsSessionNotes");
    const status = $("tsNotesSaveStatus");
    ...
    const sessionIdAtSave = capturedSessionId || currentSession.id;
    const notes = capturedNotes ?? notesInput.value;
    if(sessionIdAtSave===currentSession.id && notes===String(currentSession.notes||"")){if(status){delete status.dataset.saving;status.innerText="Saved";}return;}
    ...
    try {
      ...
      if (currentSession?.id === sessionIdAtSave) {
        ...
        try{localStorage.removeItem("ls26:notes:"+firebase.app().options.projectId+":"+sessionIdAtSave);}catch(_){}
      }
