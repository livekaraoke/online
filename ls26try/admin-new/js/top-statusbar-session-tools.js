 ...
 * admin-new/js/top-statusbar-session-tools.js — preserved application behaviour and compatibility support.
...
(() => {
  ...
  function syncNotificationsFromSession() {
    let local=[];try{local=JSON.parse(sessionStorage.getItem('ls26:notifications:'+state.sessionId)||'[]');}catch(_){}
    const log = [...(Array.isArray(state.session?.activityLog)?state.session.activityLog:[]),...local];
  ...
  async function persistActivityEntry(entry) {
    // LS26 notification feed is a bounded local journal. Actual session actions
    // still persist their authoritative state; observers never write audit noise.
    const key='ls26:notifications:'+state.sessionId;
    try{const list=JSON.parse(sessionStorage.getItem(key)||'[]');if(!list.some(x=>x.id===entry.id))sessionStorage.setItem(key,JSON.stringify([...list,entry].slice(-100)));}catch(_){}
  }
  ...
  function renderRunOrder() {
    ...
    list.innerHTML =
      ...
      items.map((item,index) => {
        const status = String(item?.status || "").toLowerCase();
        const isPlaying = status === "playing" || status === "played";
  ...
  async function acceptRequest(requestId) {
    ...
    const requestRef=state.db.collection('publicSongRequests').doc(requestId);
    const runRef=state.db.collection('karaokeControl').doc('runOrder');
    await state.db.runTransaction(async tx=>{
      const [r,q]=await Promise.all([tx.get(requestRef),tx.get(runRef)]);
      if(!r.exists)return;const request={id:r.id,...r.data()};
      if(request.sessionId!==state.sessionId||!isPendingRequest(request))return;
      const run=q.data()||{};
      if(run.sessionId&&run.sessionId!==state.sessionId)throw Error('Run Order belongs to another session. Refresh before accepting.');
      const items=Array.isArray(run.items)?run.items:[];
      if(!items.some(x=>x.requestId===requestId))items.push(makeQueueItemFromRequest(request));
      tx.set(runRef,{sessionId:state.sessionId,items,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
      tx.set(requestRef,{status:'queued',acceptedAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
    });
  }
  ...
  function startListeners() {
    ...
    state.globalUnsubs.push(
      state.db.collection("karaokeControl").doc("publicSongList").onSnapshot(doc => {
        state.publicList=doc.exists ? (doc.data() || {}) : {};
        renderPublicSetlistName(state.publicList);
        window.dispatchEvent(new Event("ls26:public-list"));
      }, console.warn)
    );

    LS26Data.collection("upcomingEvents").then(snapshot => {
      ...
      }).catch(error => console.warn("Could not load upcoming events for schedule fallback:", error));
  ...
  function ensureSongs(){
    ...
    songsLoading=LS26Data.collection("lyrics").then(snapshot => {
      ...
      renderSongSelect();
      return state.songs;
    }).catch(error=>{songsLoading=null;console.warn(error);return [];});
    return songsLoading;
  }
  ...
  function selectSessionTab(tab) {
    const map={session:['tsSessionStatusTab','tsSessionStatusPane'],notifications:['tsNotificationsTab','tsNotificationsPane'],notes:['tsNotesTab','tsNotesPane'],pending:['tsPendingTab','tsPendingPane'],runorder:['tsRunOrderTab','tsRunOrderPane']};
    if(!map[tab])tab='session';
    Object.entries(map).forEach(([name,[button,pane]])=>{
      $(button)?.classList.toggle('active',name===tab);$(button)?.setAttribute('aria-selected',String(name===tab));
      if($(pane)){$(pane).hidden=name!==tab;$(pane).classList.toggle('active',name===tab);}
    });
    try{localStorage.setItem('ls26:sessionTab',tab);}catch(_){}
    if(tab==='notifications')markNotificationsRead();
    if(tab==='runorder')ensureSongs();
  }
  function setInfoTab(tab){selectSessionTab(tab);}
  function setWorkflowTab(tab){selectSessionTab(tab);}
  function initialiseInfoTabs(){
    const bar=$('topStatusBar');if(!bar||bar.dataset.infoTabsBound)return;bar.dataset.infoTabsBound='1';
    selectSessionTab(localStorage.getItem('ls26:sessionTab')||'session');
    bar.addEventListener('click',event=>{const button=event.target.closest('[data-ts-info-tab],[data-ts-workflow-tab]');if(!button)return;event.preventDefault();event.stopPropagation();selectSessionTab(button.dataset.tsInfoTab||button.dataset.tsWorkflowTab);});
  }
  ...
  function bindUi() {
    ...
    document.addEventListener("click", event => {
      ...
      if (deleteRequestBtn) {
        return deleteRequest(deleteRequestBtn.dataset.tsDeleteRequest || deleteRequestBtn.dataset.tsDecline);
      }
