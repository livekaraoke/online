 ...
 * admin-new/js/upcoming-events.js — preserved application behaviour and compatibility support.
...
(() => {
  ...
  function eventStillUpcoming(event) {
    ...
    if (currentActiveEventId && event.id === currentActiveEventId) return false;
    if (currentActiveSessionId && event.linkedSessionId === currentActiveSessionId) return false;
  ...
  function formatDate(dateString) {
    ...
    return {
      ...
      month: date.toLocaleDateString("en-GB", { month:"short" }).toUpperCase().replace(/^SEPT$/,"SEP"),
  ...
  function nextGigStartDate(event) {
    ...
    const time = /^\d{2}:\d{2}$/.test(event.startTime) ? `${event.startTime}:00` : event.startTime;
  ...
  function tickNextGigCountdown() {
    // Work from the existing snapshot: ticking never causes a Firebase read.
    const next = findNextGig();
    const key = next ? `${next.id}|${next.date}|${next.startTime}` : "";
    if (key !== nextGigHeroKey) {
  ...
  function findNextGig(now = new Date()) {
    const today = localDateKey(now);
    return events.filter(eventStillUpcoming)
      .filter(event => {
        const start = nextGigStartDate(event);
        return start ? start.getTime() >= now.getTime() : event.date >= today;
      })
  ...
  function renderNextGigHero(event) {
    ...
    nextGigHeroKey = event ? `${event.id}|${event.date}|${event.startTime}` : "";
  ...
  function renderSummary() {
    ...
    renderNextGigHero(findNextGig(now));
  }
  ...
  function startCurrentSessionListener() {
    if (unsubscribeCurrentSession) unsubscribeCurrentSession();
    unsubscribeCurrentSession = db.collection("karaokeControl").doc("currentSession")
      .onSnapshot(doc => {
        const data = doc.exists ? (doc.data() || {}) : {};
        const active = data.active === true;
        currentActiveSessionId = active ? String(data.sessionId || data.activeSessionId || "") : "";
        currentActiveEventId = active ? String(data.eventId || data.eventSnapshot?.id || "") : "";
        renderEvents();
      }, error => {
        console.warn("Could not watch current session for Upcoming Events:", error);
      });
  }
