...
test('Song Info starts closed, contains BPM controls, resets BPM and closes on Play',()=>{
 ...
 for(const id of ['hostStickyStack','ls26StickyHeader','songInfoBtn','songInfoDrawer','closeSongInfoBtn','editSongBtn','performanceQuickInfo'])nodes.set(id,element(id));
 ...
 const karaokeRow=element('karaokeRow');nodes.get('performanceQuickInfo').querySelector=()=>karaokeRow;
 const preferences=new Map([['ls26:showKaraokeTools','false']]);
 const song={originalBpm:83,userBpm:90};let plays=0;
 const c={localStorage:{getItem:k=>preferences.get(k),setItem:(k,v)=>preferences.set(k,v)},document:{getElementById:id=>nodes.get(id),createElement:()=>element(),documentElement:{style:{setProperty(){}}},querySelector:()=>null,addEventListener:(n,f)=>events[n]=f},location:{search:'?play=1',pathname:'/host/lyricview.html'},URLSearchParams,ResizeObserver:class{observe(){}},MutationObserver:class{observe(){}},innerHeight:900,cancelAnimationFrame(){},requestAnimationFrame:()=>1,LS26Performance:{song:()=>song,setBpm:n=>song.userBpm=Math.max(1,Math.min(400,Number(n))),play(){plays++;events['ls26:song-started']()}},addEventListener:(n,f)=>events[n]=f};c.window=c;
 ...
 assert.equal(nodes.get('performanceQuickInfo').hidden,true,'restores hidden preference');nodes.get('ls26ShowKaraoke').onchange({target:{checked:true}});assert.equal(nodes.get('performanceQuickInfo').hidden,false);assert.equal(preferences.get('ls26:showKaraokeTools'),'true');karaokeRow.child.onclick();assert.equal(nodes.get('performanceQuickInfo').hidden,true);assert.equal(preferences.get('ls26:showKaraokeTools'),'false');
 ...
 assert.equal(nodes.has('ls26PlaySong'),false);assert.equal(plays,0,'even legacy play=1 URLs stay paused');events['ls26:song-started']();assert.equal(nodes.get('songInfoDrawer').classList.contains('open'),false);
});
