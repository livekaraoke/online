/************************************************************
 * LIVE KARAOKE ADMIN.JS - ADVANCED DASHBOARD
 ************************************************************/
...
auth.onAuthStateChanged(user => {
  if (user) {
    ...
    if ($("adminUserName")) $("adminUserName").innerText = user.email || "Admin";
    logAdmin("Admin logged in as: " + (user.email || "unknown"));
    initAdminAfterLogin();
  } else {
...
function updateAdminButtons(data) {
  ...
  if ($("liveToggleBtn")) $("liveToggleBtn").innerText = live ? "🔴 END LIVE" : "🟢 GO LIVE";
  if ($("songsToggleBtn")) $("songsToggleBtn").innerText = !live ? "🔒 SONGS LOCKED" : songs ? "🔴 LOCK SONGS" : "🟢 UNLOCK SONGS";
}
...
function updateStatusStrip() {
  ...
  if ($("statusLiveLabel")) $("statusLiveLabel").innerText = live ? "● LIVE" : "● Offline";
  ...
  if ($("statusSongsLabel")) $("statusSongsLabel").innerText = songs ? "UNLOCKED" : "LOCKED";
...
function toggleSongsAuto() {
  ...
  if (!live) {
    ...
    logAdmin("Cannot unlock songs: event is not live"); return;
  }
  const songs = getSongsAvailable(currentState, live);
  db.collection("karaoke").doc("state").set({ songsOverride: !songs, songsEnabled: !songs }, { merge: true });
  logAdmin(songs ? "Songs LOCKED manually" : "Songs UNLOCKED");
}
...
async function confirmStartPerformance() {
  const ok = await showConfirm("Start Performance?", "This will start a new performance session and attach new song requests to it.");
  if (ok) startPerformance();
}
async function confirmEndPerformance() {
  const ok = await showConfirm("End Performance?", "This will end the current session. Are you sure?");
  if (ok) endPerformance();
}
async function startPerformance() {
  ...
  const ref = await db.collection("performanceSessions").add({
    title, venue, notes, status: "active", isActive: true, breakOpen: false,
    startedAt: serverNow(), endedAt: null, breaks: [], createdAt: serverNow(), updatedAt: serverNow()
  });
...
function calculateBreakMs(session) {
  ...
  (session?.breaks || []).forEach(b => {
    const start = getDateFromTimestamp(b.start); const end = getDateFromTimestamp(b.end) || new Date();
...
function initAdminAfterLogin() {
  listenKaraokeState(); listenCurrentSession();
